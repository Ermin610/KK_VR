// Taken from the KK_MainGameVR / KKS_VR plugins (MIT, see ThirdPartyNotices/KoikatuVR-MIT.txt),
// IllusionMods/KKS_VR commit f2f8b45b8f8ab100a099d4b39b0e23126af35f26, along with the
// mirror-shader bundle beside this file. Originally adapted from
...
namespace KKCharaStudioVR.Mirror
{
    public class VRReflection : MonoBehaviour
    {
        ...
        // Fraction of the source camera resolution the per camera textures are allocated at.
        // Full eye resolution per mirror per camera costs far more fill than the mirror is
        // worth, and half of it is soft enough to notice. Overridden from the settings file.
        public float m_ResolutionScale = DefaultResolutionScale;

        public const float DefaultResolutionScale = 0.75f;

        // Nudges the oblique near plane behind the mirror surface, as stock MirrorReflection does.
        ...
        // Called when the object is about to be rendered by some camera.
        public void OnWillRenderObject()
        {
            var rend = GetComponent<Renderer>();
            if (!enabled || !rend || !rend.enabled || !rend.sharedMaterial)
                return;
            ...
            if (cam.stereoEnabled)
            {
                if (cam.stereoTargetEye == StereoTargetEyeMask.Both || cam.stereoTargetEye == StereoTargetEyeMask.Left)
                {
                    ...
                    RenderMirror(cam, reflectionData.camera, target, eyePos, eyeRot, projectionMatrix);
                }
            ...
            else
            {
                var target = m_UseSharedRenderTexture ? m_SharedReflectionTextureLeft : reflectionData.left;
                reflectionData.propertyBlock.SetTexture(s_LeftTexturePropertyID, target);
                RenderMirror(cam, reflectionData.camera, target, cam.transform.position, cam.transform.rotation, cam.projectionMatrix);
            }
        ...
        private void RenderMirror(UnityEngine.Camera sourceCamera, UnityEngine.Camera reflectionCamera, RenderTexture targetTexture, Vector3 camPosition, Quaternion camRotation, Matrix4x4 camProjectionMatrix)
        {
            ...
            // never render water layer, and only what the source camera itself draws so the
            // studio axis and collider layers stay in step with the room
            reflectionCamera.cullingMask = ~(1 << 4) & m_ReflectLayers.value & sourceCamera.cullingMask;
        ...
        private void CreateMirrorObjects(UnityEngine.Camera currentCamera, out ReflectionData reflectionData)
        {
            ...
            else
            {
                ...
                int width, height;
                ReflectionSize(currentCamera, out width, out height);

                // Reflection render texture
                if (!reflectionData.left || reflectionData.left.width != width || reflectionData.left.height != height)
                {
                    if (reflectionData.left)
                        DestroyImmediate(reflectionData.left);
                    reflectionData.left = new RenderTexture(width, height, 24);
                ...
                if (currentCamera.stereoEnabled)
                    if (!reflectionData.right || reflectionData.right.width != width || reflectionData.right.height != height)
                    {
                        if (reflectionData.right)
                            DestroyImmediate(reflectionData.right);
                        reflectionData.right = new RenderTexture(width, height, 24);
        ...
        private void ReflectionSize(UnityEngine.Camera currentCamera, out int width, out int height)
        {
            var settings = VR.Manager.Context.Settings as KKCharaStudioVRSettings;
            if (settings != null)
                m_ResolutionScale = settings.MirrorResolutionScale;

            var scale = Mathf.Clamp(m_ResolutionScale, 0.1f, 1f);
            width = Mathf.Max(16, Mathf.RoundToInt(currentCamera.pixelWidth * scale));
            height = Mathf.Max(16, Mathf.RoundToInt(currentCamera.pixelHeight * scale));
        }
